$(document).ready(function () {
    ...
    $.createElement = function (data, nutritionFacts) {
        ...
        dish.push(product);
        updateTotals(proteins, carbs, fats, energy, weightOfProduct, '+');
   ...
   function initialize() {
        var rows = $('#DishesBody > tr');

        rows.each(function () {
            var proteins = parseFloat($(this).find('.Proteins').text());
            var carbs = parseFloat($(this).find('.Carbohydrates').text());
            var fats = parseFloat($(this).find('.Fat').text());
            var energy = parseFloat($(this).find('.Energy').text());
            var weightOfProduct = parseFloat($(this).find('.ProductWeight').text());

            var product = {};
            product["lp"] = $(this).find('.FoodProductNo').text();
            product["foodType"] = $(this).find('.ProductType').text();
            product["name"] = $(this).find('.ProductName').text();
            product["weight"] = weightOfProduct;
            product["protein"] = proteins;
            product["carbohydrates"] = carbs;
            product["fat"] = fats;
   ...
   function updateTotals(proteins, carbs, fat, energy, weight, type) {
        if (type === '+') {
            totalWeight += weight;
